...
public class SpawnManager : MonoBehaviour
{
    ...
    public void SpawnItemDrop()
    {
        ...
        foreach (var pair in itemPairs)
        {
            for (var i = 0; i < pair.Item2; i++)
            {
                ...
                newItem.Spawn(itemPos, spawnDelay, spawnScaleDuration);
    ...
    public Amo DropAmo(AmoType type, int amount, Vector2 startPos, Vector2 endPos)
    {
        //startPos = endPos = Vector2.zero;

        var newAmo = GetAmo(type, amount, startPos);

        if (newAmo != null)
        {
            newAmo.Spawn(endPos,0,spawnScaleDuration);
            return newAmo;
        }
    ...
    public void FillCharacterCapWithBots ()
    {
        ...
        for (int i = 0; i < botAmount; i++)
        {
            ...
            spawnCharacter.SpawnWithType(botPos, Character.CharacterType.localBot);
        }
    }
